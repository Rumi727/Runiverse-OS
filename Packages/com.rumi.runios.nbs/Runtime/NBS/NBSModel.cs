...
namespace RuniOS.NBS
{
    ...
    /// </summary>
    public sealed class NBSFile(NBSHeader header, IReadOnlyList<NBSTick> ticks, IReadOnlyList<NBSLayer> layers, IReadOnlyList<NBSCustomInstrument> customInstruments, IReadOnlyList<NBSSpecialEvent> specialEvents, int tickLength)
    {
        /// <summary>Gets the complete file header.<br/>파일 헤더 전체를 가져옵니다.</summary>
        public NBSHeader header { get; } = header;

        /// <summary>Gets active tick columns in ascending order.<br/>활성 틱 열을 오름차순으로 가져옵니다.</summary>
        public IReadOnlyList<NBSTick> ticks { get; } = ticks;

        /// <summary>Gets layer metadata.<br/>레이어 메타데이터를 가져옵니다.</summary>
        public IReadOnlyList<NBSLayer> layers { get; } = layers;

        /// <summary>Gets custom instrument definitions.<br/>커스텀 악기 정의를 가져옵니다.</summary>
        public IReadOnlyList<NBSCustomInstrument> customInstruments { get; } = customInstruments;
        ...
        public IReadOnlyList<NBSSpecialEvent> specialEvents { get; } = specialEvents;

        /// <summary>Gets the tempo map used by transport and scheduling.<br/>트랜스포트와 예약에 사용하는 템포 맵을 가져옵니다.</summary>
        public NBSTempoMap tempoMap { get; } = new NBSTempoMap(header.ticksPerSecond, specialEvents);

        /// <summary>
        /// Gets the visual-effect map used by editor previews.<br/>
        /// 에디터 미리보기에 사용하는 시각 효과 맵을 가져옵니다.
        /// </summary>
        public NBSVisualEffectMap visualEffectMap { get; } = new NBSVisualEffectMap(specialEvents);

        /// <summary>Gets the logical song length in NBS ticks.<br/>NBS 틱 단위의 논리적 곡 길이를 가져옵니다.</summary>
        public int tickLength { get; } = tickLength;
