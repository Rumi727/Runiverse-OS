...
namespace RuniOS
{
    ...
    public static class SerializableKeyValuePair
    {
        ...
        /// </returns>
        public static (Type? key, Type? value) GetUnderlyingType(Type nullableType)
        {
            // 제네릭 타입이고 제네릭 정의가 아닌 경우에만 처리합니다.
            if (nullableType.IsGenericType && !nullableType.IsGenericTypeDefinition)
            {
                // ISerializableNullable<>의 제네릭 정의에 할당 가능한지 확인합니다.
                if (nullableType.IsAssignableToGenericDefinition(typeof(ISerializableKeyValuePair<,>), out Type? resolvedType))
                {
