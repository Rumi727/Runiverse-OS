...
{
    /// <summary>
    /// <see cref="RuniPath"/> 구조체를 JSON 문자열로 직렬화하고 역직렬화하는 <see cref="JsonConverter"/>입니다.
    /// <br/>
    /// 이 컨버터는 <see cref="RuniPath"/> 인스턴스를 해당 문자열 표현으로 변환하거나,
    /// JSON 문자열을 <see cref="RuniPath"/> 인스턴스로 파싱하는 역할을 합니다.
    /// </summary>
    public class RuniPathConverter : JsonConverter<RuniPath>
    {
        /// <summary>
        /// <see cref="RuniPath"/> 객체를 JSON 문자열로 직렬화합니다.
        /// <br/>
        /// <see cref="RuniPath"/>의 <see cref="RuniPath.ToString"/> 메서드를 사용하여 경로를 문자열로 기록합니다.
        /// </summary>
        /// <param name="writer">JSON 작성을 위한 <see cref="JsonWriter"/> 객체입니다.</param>
        /// <param name="value">직렬화할 <see cref="RuniPath"/> 값입니다.</param>
        /// <param name="serializer">직렬화 프로세스를 위한 <see cref="JsonSerializer"/> 객체입니다.</param>
        public override void WriteJson(JsonWriter writer, RuniPath value, JsonSerializer serializer) => writer.WriteValue(value.ToString());
        ...
        /// <returns>역직렬화된 <see cref="RuniPath"/> 객체입니다.</returns>
        ...
        {
            JsonToken.String => RuniPath.From((string?)reader.Value ?? string.Empty),
