...
namespace RuniEngine.Modding
{
    public static class HarmonyUtility
    {
        ...
        /// <remarks>
        /// <para>Unity 에디터에서 실행될 경우, 이 메서드는 <see cref="Harmony.PatchAll"/> 호출을 몇 프레임 지연시킵니다.
        /// 이 지연은 에디터 시작 또는 특정 에디터 이벤트 중 즉각적인 패치로 인해 발생할 수 있는 충돌이나 문제를 방지하는 데 도움이 될 수 있습니다.</para>
        /// <para>런타임(빌드된 게임) 중에는 동일한 에디터 관련 고려 사항이 적용되지 않으므로 <see cref="Harmony.PatchAll"/>가 지연 없이 즉시 호출됩니다.</para>
