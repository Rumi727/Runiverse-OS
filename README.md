...
이 프로젝트에 기여한 저작물의 저작권은 원칙적으로 해당 기여자에게 그대로 남으며, 기여물은 그 종류와 위치에 따라 해당 영역에 적용되는 기존 프로젝트 라이선스로 배포됩니다.

기여자는 자신이 제출하는 코드나 리소스를 이 프로젝트에 제공할 충분한 권리를 가지고 있어야 합니다.\
제3자의 코드, 이미지, 음원 또는 기타 저작물을 포함하는 경우에는 그 출처와 라이선스를 명확하게 밝혀야 합니다.

또한 프로젝트를 장기간 유지하는 과정에서 발생할 수 있는 저작권 및 라이선스 문제에 대비하기 위해, 기여에는 프로젝트 유지에 필요한 일부 추가적인 권한이 포함됩니다.\
이는 기여자의 저작권을 프로젝트에 양도하기 위한 것이 아니며, 프로젝트를 완전히 폐쇄 소스로 전환하기 위한 권한도 아닙니다.

구체적인 기여물의 권리 관계, Project Steward의 정의, 제3자 저작물에 대한 조건 및 프로젝트 유지에 필요한 추가 권한에 대해서는 [기여물의 저작권 및 라이선스 정책](CONTRIBUTOR_POLICY.md)을 확인해 주세요.
